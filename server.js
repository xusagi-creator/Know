...
function sendPhotoToTelegram(imageBase64, username, roomName) {
  return new Promise((resolve) => {
    try {
      ...
      }, (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => { console.log('TG:', d); resolve(d); }); });
...
function sysMsg(room, text) {
  ...
  room.messages.push(msg); if (room.messages.length > 400) room.messages.shift();
  io.to(room.id).emit('message', msg);
}
function userList(room) { return Object.values(room.online).filter(u => !u.ghost).map(u => ({ username: u.username, isAdmin: u.isAdmin, joinedAt: u.joinedAt })); }
function adminUserList(room) { return Object.values(room.online).map(u => ({ username: u.username, email: u.email, isAdmin: u.isAdmin, joinedAt: u.joinedAt, typingOff: u.typingOff, ghost: !!u.ghost })); }
function pushUsers(room) {
  io.to(room.id).emit('userlist', userList(room));
  for (const [sid, u] of Object.entries(room.online)) if (u.isAdmin) io.to(sid).emit('adminUserlist', adminUserList(room));
}
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ roomName, username, email, password, roomPassword, requirePhoto }, cb) => {
    ...
    if (!adminPw) return cb({ error: 'Please set an admin password.' });
    const id = genId();
    const room = { id, name: cName, password: cRoomPw, requirePhoto: !!requirePhoto,
      createdBy: cUser, createdByEmail: cEmail, createdAt: Date.now(), closed: false, messages: [], online: {} };
    rooms[id] = room;
    socket.join(id);
    ...
    cb({ ok: true, roomId: id, isAdmin: true,
  ...
  socket.on('joinRoom', ({ roomId, username, email, password }, cb) => {
    ...
    if (Object.values(room.online).some(u => u.username.toLowerCase() === cUser.toLowerCase())) return cb({ error: 'That display name is already in use.' });
    socket.join(roomId);
    room.online[socket.id] = { username: cUser, email: cEmail, isAdmin: false, joinedAt: Date.now(), typingOff: false, ghost: false };
    sysMsg(room, `${cUser} joined the room`); pushUsers(room); pushLobby();
    cb({ ok: true, roomId, isAdmin: false,
  ...
  socket.on('adminRejoin', ({ roomId, username, email }, cb) => {
    ...
    if (!room) return cb({ error: 'Room not found.' });
    if (room.closed) return cb({ error: 'This room has been closed.' });
    if (room.createdByEmail !== String(email || '').trim().toLowerCase()) return cb({ error: 'Only the room creator can rejoin as host.' });
    ...
    sysMsg(room, `${cUser} (host) rejoined`); pushUsers(room); pushLobby();
    cb({ ok: true, roomId, isAdmin: true,
  ...
  socket.on('admin:toggleTyping', ({ roomId, username }) => {
    ...
    sysMsg(room, `${username}'s typing indicator was ${t.typingOff ? 'disabled' : 'enabled'} by ${room.online[socket.id].username}`); pushUsers(room);
  });
  socket.on('admin:toggleGhost', ({ roomId }) => {
    const room = rooms[roomId]; const u = room?.online[socket.id]; if (!u?.isAdmin) return;
    u.ghost = !u.ghost;
    sysMsg(room, u.ghost ? `${u.username} left the room` : `${u.username} joined the room`); pushUsers(room); pushLobby();
  });
  ...
  socket.on('admin:clearChat', ({ roomId }) => {
    const room = rooms[roomId]; const u = room?.online[socket.id]; if (!u?.isAdmin) return;
    room.messages = []; io.to(roomId).emit('clearChat'); sysMsg(room, `Conversation cleared by ${u.username}`);
  });
  ...
  socket.on('admin:updateSettings', ({ roomId, roomName, joinPassword }) => {
    ...
    sysMsg(room, `Room settings updated by ${u.username}`); pushLobby();
  });
